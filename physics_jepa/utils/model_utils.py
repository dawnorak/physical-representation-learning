...
class ResidualBlock2Plus1D(nn.Module):
    """
    Factorized depthwise residual block for 3D features:
    temporal mixing (k_t,1,1) followed by spatial mixing (1,k,k).
    """

    def __init__(self, embed_dim, layer_scale_init_value=1e-6, drop_path=0.):
        super().__init__()
        self.temporal_conv = nn.Conv3d(
            embed_dim, embed_dim, kernel_size=(3, 1, 1), padding=(1, 0, 0), groups=embed_dim
        )
        self.spatial_conv = nn.Conv3d(
            embed_dim, embed_dim, kernel_size=(1, 7, 7), padding=(0, 3, 3), groups=embed_dim
        )
        self.norm = LayerNorm(embed_dim, data_format="channels_last")
    ...
    def __init__(self,
        ...
        if num_frames == 16:
            ...
            for i in range(len(dims)):
                if self.encoder_block_type == "factorized_2plus1d" and i < len(dims) - 1:
                    block_cls = ResidualBlock2Plus1D
                    block_kwargs = {}
                elif self.encoder_block_type in ("standard", "factorized_2plus1d"):
                    block_cls = ResidualBlock
                    block_kwargs = {"num_spatial_dims": 3 if i < len(dims)-1 else 2}
                else:
                    raise ValueError(
                        f"Unsupported encoder_block_type: {self.encoder_block_type}. "
                        "Choose from {'standard', 'factorized_2plus1d'}."
                    )
            ...
            self.res_blocks = nn.ModuleList()
            for i in range(len(dims)):
                if self.encoder_block_type == "factorized_2plus1d" and i < 3:
                    block_cls = ResidualBlock2Plus1D
                    block_kwargs = {}
                elif self.encoder_block_type in ("standard", "factorized_2plus1d"):
                    block_cls = ResidualBlock
                    block_kwargs = {"num_spatial_dims": 3 if i < 3 else 2}
                else:
                    raise ValueError(
                        f"Unsupported encoder_block_type: {self.encoder_block_type}. "
                        "Choose from {'standard', 'factorized_2plus1d'}."
